...
- `POST /api/v1/token/refresh/access` expects `refreshToken` cookie and returns renewed cookies
  - the Andromeda auth service issues single-use, rotating refresh tokens with replay
    detection, so a structurally valid refresh token can still be rejected once it has
    been rotated, revoked, or is otherwise no longer active
  - when the auth service responds with `401`, Nebula returns `401 UNAUTHORIZED`
    (re-authentication required), **not** `502 BAD_GATEWAY`; only genuine gateway
    failures (connectivity and upstream `5xx`) map to `502 BAD_GATEWAY`
...
- public Actuator: `/api/actuator/health`, `/api/actuator/info`, `/api/actuator/prometheus`
- authenticated Actuator: remaining `/api/actuator/**`
...
2. **Controller-level status returns and central exception handling**
   ...
   - `AuthServiceUnauthorizedException` -> `401 UNAUTHORIZED` (auth service rejected access-token refresh)
   - `HttpRequestException` -> `502 BAD_GATEWAY` (auth service unreachable or upstream `5xx`)
