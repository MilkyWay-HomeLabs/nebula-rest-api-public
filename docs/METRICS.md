...
Actuator access is configured in `AppConfig.securityFilterChain(...)`:

- `/api/actuator/health/**`, `/api/actuator/info`, and `/api/actuator/prometheus` are public
- all remaining `/api/actuator/**` endpoints require authentication

This matches the `management.endpoint.prometheus.access=unrestricted` property and allows Prometheus to scrape metrics directly without requiring application user authentication tokens.

### Recommendation

For production, keep Prometheus scraping behind trusted infrastructure and network policies. If you want stricter hardening, also review network policy, ingress rules, and whether `/api/actuator/metrics` should remain accessible to authenticated application users.
