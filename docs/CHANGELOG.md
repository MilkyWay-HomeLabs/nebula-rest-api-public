...
### Security
- Sanitize cookies in the refresh token response to prevent stale cookie issues.
- Honor the `CORS_ALLOWED_ORIGIN_PATTERNS` environment variable in the default profile.
- Allow public access to `/api/actuator/prometheus` so the endpoint can be scraped without authentication.

### CI/CD
- Add a production deploy workflow for K3s (prod).
- Update the deploy workflow to use the current Tomcat container name.
- Report build status to the MilkyWay Hub after tests (P4-2).
- Report documentation presence (README) to the MilkyWay Hub (P5-1).

### Documentation
- Add the Authentication & Token Integration Guide (`docs/AUTHENTICATION.md`).

### Internal
- Refactor test configurations and dependencies for improved database testing.

## v 5.4.1-PUBLIC-PREVIEW (2026-07-11)

### Fixed
- Fix access-token refresh reporting a misleading `502 BAD_GATEWAY` when the Andromeda
  authentication service rejects the refresh token with `401`. Because Andromeda issues
  single-use, rotating refresh tokens with replay detection, a token that is still
  structurally valid can legitimately be rejected once it has been rotated, revoked, or
  is otherwise no longer active (for example, after concurrent refresh calls or a stale
  cookie). Nebula previously collapsed every non-2xx auth-service status into
  `HttpRequestException` → `502 BAD_GATEWAY`, hiding the real cause from the client.
- `POST /api/v1/token/refresh/access` now returns `401 UNAUTHORIZED` when the auth
  service responds with `401`, allowing the frontend to redirect to login instead of
  treating the failure as an upstream outage. Genuine gateway failures (connectivity
  issues and upstream `5xx`) still return `502 BAD_GATEWAY`.

### Internal
- Add `AuthServiceResponseException` (carries the upstream HTTP status code) and
  `AuthServiceUnauthorizedException` to distinguish auth-service authorization
  rejections from gateway failures in `HttpAuthClient`.
- Map `AuthServiceUnauthorizedException` to `401 UNAUTHORIZED` in `GlobalExceptionHandler`.

### Testing
- Add `HttpAuthClientTest` coverage asserting that a `401` from the auth-service
  refresh endpoint raises `AuthServiceUnauthorizedException`, while a `5xx` still raises
  `HttpRequestException`.
- Add `GlobalExceptionHandlerTest` coverage for the new `401 UNAUTHORIZED` mapping.
