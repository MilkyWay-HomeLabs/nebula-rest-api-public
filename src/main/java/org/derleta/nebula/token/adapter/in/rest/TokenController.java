...
public final class TokenController {
    ...
    //
    // The full Set-Cookie strings (Domain/Max-Age/SameSite included) are forwarded as-is,
    // matching how AccountController#getToken forwards login cookies. Rebuilding the cookie
    // from scratch here previously dropped Domain and Max-Age, which made the browser store
    // the refreshed cookie as a separate host-only/session cookie instead of replacing the
    // one issued at login — the stale login cookie then kept winning on subsequent requests.
    ...
    private ResponseEntity<Response> getResponseForRefreshAccess(Response response) {
        if (response instanceof AccessResponse instance) {
            ...
            String accessCookie = cookies.get("accessToken");
            if (accessCookie != null) {
                builder.header(HttpHeaders.SET_COOKIE, CookieSanitizer.sanitize(accessCookie));
            }
            String refreshCookie = cookies.get("refreshToken");
            if (refreshCookie != null) {
                builder.header(HttpHeaders.SET_COOKIE, CookieSanitizer.sanitize(refreshCookie));
            }

            return builder.body(instance);
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
    }
